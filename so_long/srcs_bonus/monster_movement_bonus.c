...
static char	monster_move_up(t_data *data, t_map *m, int monster_movement)
{
	...
	if (monster_movement % 5 == 0)
		...
		mlx_put_image_to_window(data->mlx, data->win, data->m.img,
			m->monster_x * 64, m->monster_y * 64);
	data->map.map[m->monster_y][m->monster_x] = 'm';
	return ('u');
}
...
void	move_monster(t_data *data, t_map *m)
{
	...
	if (data->map.map[m->monster_y - 1][m->monster_x] == 'm' && c != 'd')
		c = monster_move_up(data, m, monster_movement);
	else if (data->map.map[m->monster_y + 1][m->monster_x] == 'm' && c != 'u')
		c = monster_move_down(data, m, monster_movement);
	else if (data->map.map[m->monster_y][m->monster_x + 1] == 'm' && c != 'l')
		c = monster_move_right(data, m, monster_movement);
	else if (data->map.map[m->monster_y][m->monster_x - 1] == 'm' && c != 'r')
		c = monster_move_left(data, m, monster_movement);
	else if (c != 0)
	{
		c = 0;
		move_monster(data, m);
